...
#else
    searchPaths = [NSArray arrayWithObject:@"/usr/local/share/libFoundation"];
#endif
...
{
//NSLog(@"in load_callback, debugBundles = %d", debugBundles);
...
- (BOOL)loadCode
{
    ...
    if ((file = [[self infoDictionary] objectForKey:@"CFBundleExecutable"]) != nil) {
...
#endif
#endif
//NSLog(@"loading module %s", [file fileSystemRepresentation]);
...
+ (NSBundle *)bundleWithIdentifier:(NSString *)identifier
{
    ...
    NSMapEnumerator e = NSEnumerateMapTable(bundleNames);
    while (NSNextMapEnumeratorPair(&e, (void**)&key, (void**)&value)) {
//	NSLog(@"bundle dictionary: %@", [[value infoDictionary] description]);
	NSString *bundleIdentifier = [[value infoDictionary] objectForKey:@"CFBundleIdentifier"];
//	NSLog(@"bundle identifier %@", bundleIdentifier);
	if ([bundleIdentifier isEqual:identifier])
	    return value;
    }
